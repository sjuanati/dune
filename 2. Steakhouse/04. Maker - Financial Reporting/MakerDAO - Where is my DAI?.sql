WITH
  addresses AS (
    ...
    UNION ALL
    SELECT 
        0x143cc0a996de329c1c5723ee4f15d2a40c1203c6 as wallet,
        'Treasury' as wallet_type,
        'Movement DAO' AS protocol
    UNION ALL
    SELECT 0x3e40d73eb977dc6a537af587d48316fee66e9c8c as wallet,
        'Treasury' as wallet_type,
        'Lido' as protocol
    UNION ALL
    SELECT 0x5db0761487e26B555F5Bfd5E40F4CBC3E1a7d11E as wallet,
        'Treasury' as wallet_type,
        'Olympus' as protocol
    UNION ALL
    SELECT 0x1715a3E4A142d8b698131108995174F37aEBA10D as wallet,
        'Bridge' as wallet_type,
        'Pulsechain' as protocol
    UNION ALL
    SELECT 0x3154Cf16ccdb4C6d922629664174b904d80F2C35 as wallet,
        'Bridge' as wallet_type,
        'Base' as protocol
    UNION ALL
    SELECT 0x1116898DdA4015eD8dDefb84b6e8Bc24528Af2d8 as wallet,
        'Bridge' as wallet_type,
        'Synapse' as protocol
    UNION ALL
    SELECT 0x018008bfb33d285247A21d44E50697654f754e63 as wallet,
        'Lending' as wallet_type,
        'Aave' as protocol
    UNION ALL
    SELECT 0x3ee18B2214AFF97000D974cf647E7C347E8fa585 as wallet,
        'Bridge' as wallet_type,
        'Wormhole' as protocol
    UNION ALL
    SELECT 0xC3D03e4F041Fd4cD388c549Ee2A29a9E5075882f as wallet,
        'Dex' as wallet_type,
        'Sushiswap' as protocol
    UNION ALL
    SELECT 0x99cEF4A71d42C131fd20c0f24f30b0Eb0b22792C as wallet,
        'Treasury' as wallet_type,
        'Decentral Games' as protocol
    UNION ALL
    SELECT 0x6Ef682F0223687c625E6c4a115F544a80c37dA33 as wallet,
        'Treasury' as wallet_type,
        'Tangible' as protocol
    UNION ALL
    SELECT 0x27F8D03b3a2196956ED754baDc28D73be8830A6e as wallet,
        'Lending' as wallet_type,
        'Aave' as protocol
    UNION ALL
    SELECT 0xcafea112Db32436c2390F5EC988f3aDB96870627 as wallet,
        'Treasury' as wallet_type,
        'Nexus Mutual' as protocol
    UNION ALL
    SELECT 0x82E64f49Ed5EC1bC6e43DAD4FC8Af9bb3A2312EE as wallet,
        'Lending' as wallet_type,
        'Aave' as protocol
  ),
  deltas AS (
    ...
    SELECT
      ...
      CAST("rad" AS DOUBLE) / CAST(POWER(10, 27) AS DOUBLE) AS delta
    FROM
...
/*
grouped as (
    select wallet, dt, sum(delta) as delta
    from deltas
    where wallet <> '\x0000000000000000000000000000000000000000'
    group by 1, 2
),
balances as (
    select wallet, wallet_type, dt, sum(delta) over(partition by wallet order by dt asc) as balance
    from grouped
    left join addresses using (wallet)
)
select wallet, wallet_type, balance/10^18
from balances
where dt = '2021-12-01'
order by 3 desc
*/