...
WITH
  persons AS (
    ...
    UNION ALL
    SELECT
      0x59cee32f3ffaeabc0d4991ac7569ebad09e1a7d4 AS address,
      'Parafi' AS name
    UNION ALL
    SELECT
      0x0179437cc2f6c3812f48f9376b708c481a725e3f AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0x15232c8ceda3293ed5984d4631d4ad95ec39db19 AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0x60ffdd5f4735e13c9bc805c4d26f76b0952c2000 AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0x979423ab7b42161c31e6c303a4850a454716ce86 AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0x9ee6fce533e27a546e07b3b93c7952da00a87d5e AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0xf88790fb8f155924a2b3519871711f776a3cd0ff AS address,
      'Standard Crypto' AS name
    UNION ALL
    SELECT
      0x58f5f0684c381fcfc203d77b2bba468ebb29b098 AS address,
      'BlockTower' AS name
    UNION ALL
    SELECT
    ...
    UNION ALL
    SELECT
      0xca8d6f69d8f32516a109df68b623452cc9f5e64d AS address,
      'Correlation Fish' AS name
    UNION ALL
    SELECT
      0x11577a8a5baf1e25b9a2d89f39670f447d75c3cd AS address,
      'Correlation Fish' AS name
    UNION ALL
    SELECT
      0xf37216a8ac034d08b4663108d7532dfcb44583ed AS address,
      'Correlation Fish' AS name
    UNION ALL
    SELECT
      0xefcc3401739427eb0491cc27c7baa06817c7dfdb AS address,
  ...
  stackers AS (
    SELECT
      ...
      cast(value as int256) / CAST(POWER(10, 18) AS DOUBLE) AS power
    FROM
  ...
  fill_gaps_2 AS (
    ...
    FROM
      fill_gaps_1
      CROSS JOIN UNNEST (SEQUENCE(min_dt, CURRENT_DATE, INTERVAL '1' day)) AS _u (dt)
  ),
  fill_gaps AS (
    SELECT
      delegate,
