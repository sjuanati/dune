...
excluded_addresses(address, address_type) as (
    VALUES
    -- https://github.com/curvefi/curve-stablecoin/blob/master/deployment-logs/mainnet.log
    (0xaA346781dDD7009caa644A4980f044C50cD2ae22, 'PegKeeper')
    , (0xE7cd2b4EB1d98CD6a4A48B6071D46401Ac7DC5C8, 'PegKeeper')
    , (0x6B765d07cf966c745B340AdCa67749fE75B5c345, 'PegKeeper')
    , (0x1ef89Ed0eDd93D1EC09E4c07373f69C49f4dcCae, 'PegKeeper')
    , (0x8472A9A7632b173c8Cf3a86D3afec50c35548e76, 'Controller')
    , (0xa920de414ea4ab66b97da1bfe9e6eca7d4219635, 'Controller')
    , (0x4e59541306910aD6dC1daC0AC9dFB29bD9F15c67, 'Controller')
    , (0x100dAa78fC509Db39Ef7D04DE0c1ABD299f4C6CE, 'Controller')
    , (0xEC0820EfafC41D8943EE8dE495fC9Ba8495B15cf, 'Controller')
    , (0x1C91da0223c763d2e0173243eAdaA0A2ea47E704, 'Controller')
...
(
    ...
    (
        ...
        , (0xCFFAd3200574698b78f32232aa9D63eABD290703, 'CeFi',  'Crypto.com')
        ...
        , (0xae2d4617c862309a3d75a0ffb358c7a5009c673f, 'CeFi', 'Kraken') /* pretty much just usdc in here */
        ...
        , (0x61fA2c947e523F9ABfb8d7e2903A5D5218C119a7, 'DEX', 'Curve.Fi')
        , (0xf3B0073E3a7F747C7A38B36B805247B222C302A3, 'CeFi', 'Crypto.com')
...
(
    ...
    (
        ...
        , (0x0a59649758aa4d66e25f08dd01271e891fe52199, 'Maker USDC PSM', 'PSM')
        ...
        , (0xa191e578a6736167326d05c119ce0c90849e84b7, 'Maker USDC Vault', 'Lending')
        ...
        , (0x000000000dFDe7deaF24138722987c9a6991e2D4, 'Someones MEV Bot', 'MEV Bot')
        , (0x000000000dFDe7deaF24138722987c9a6991e2D4, 'Uniswap', 'DEX')
        , (0x9a22cdb1ca1cdd2371cd5bb5199564c4e89465eb, 'curvefi', 'DEX')
        , (0x07af6bb51d6ad0cf126e3ed2dee6eac34bf094f8, 'fraxswap', 'DEX')
        , (0x3154Cf16ccdb4C6d922629664174b904d80F2C35, 'Base', 'Bridge')
        , (0x96eFdF95Cc47fe90e8f63D2f5Ef9FB8B180dAeB9, 'Silo', 'Lending')
        , (0xb0823c25cDF531a58e581eE14f160c290fef5722, 'SiloV2', 'Lending')
        , (0x13394005C1012e708fCe1EB974F1130fDc73a5Ce, 'Uniswap', 'DEX')
        , (0xc30C8b862f7DE6bA5d7Eaeb113c78EC6B5dED04b, 'Uniswap', 'DEX')
        , (0xf89d7b9c864f589bbF53a82105107622B35EaA40, 'Bybit', 'CeFi')
        , (0xE25a329d385f77df5D4eD56265babe2b99A5436e, 'Paxos', 'Treasury')
        , (0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, 'Gate.io', 'CeFi')
        , (0x48EC5560bFD59b95859965cCE48cC244CFDF6b0c, 'Bitstamp', 'CeFi')
        , (0xd3F5c377A8506af29AEAfBc90D1E5C6534f197b4, 'Uniswap', 'DEX')
        , (0x857110b5f8efd66cc3762abb935315630ac770b5, 'Curve.Fi', 'DEX')
        , (0x264bd8291fAE1D75DB2c5F573b07faA6715997B5, 'Paxos', 'CeFi')
        , (0xad6eaa735D9dF3D7696fd03984379dAE02eD8862, 'Cumberland', 'Treasury')
        , (0xC4334A9AF50C80A12C484de643149f6159Bdd110, 'BingX', 'CeFi')
...
(
    ...
    , CASE WHEN name IN ('OlympusTreasury','MapleTreasury','Proxy','Proxy_v2','Treasury') THEN 'Treasury' /* potentially includes personal smart contract wallets */
        -- WHEN name IN ('Curve') or namespace in ('curvefi') THEN 'Curve'
        WHEN name IN ('Pair','Fei3Crv','LiquidityPoolV4','DMMPool','UniswapV2Pair','DEX','YPoolDelegator','AMMWrapper','SigThreePool') THEN 'DEX'
        ...
        WHEN namespace IN ('balancer_v2','saddle','dodo','bancor','bancor3','balancer','defiswap','temple','shell_v1'
...
(
    ...
    CROSS JOIN 
    UNNEST(SEQUENCE(DATE('2020-01-01'), CURRENT_DATE, INTERVAL '1' DAY)) AS t(dt)
), grouped AS 
(
    SELECT wallet
    , dt
...
(
    SELECT wallet
    , dt
    , SUM(delta) OVER (PARTITION BY wallet ORDER BY dt) / 1e6 AS balance
    FROM grouped
)

SELECT dt, wallet, CASE WHEN balance <= 1e-6 THEN 0 ELSE balance END balance 
-- , total_balance
from (
    SELECT wallet
    , dt
