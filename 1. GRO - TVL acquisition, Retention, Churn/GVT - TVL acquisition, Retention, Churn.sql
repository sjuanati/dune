...
-- version 3.0 [GVT now based on realised TVL]
WITH
    ...
    -- Utilisation Ratio & GVT totalAssets from PnL
    pnl AS (
        SELECT "week" AS "week",
        ...
        FROM (
            SELECT pnl_dates.week AS "week",
                   pnl."afterGvtAssets" / 1e18 AS "gvt_assets",
    ...
    -- GVT price on previous week based on totalAssets / totalSupply from PnL
        ...
        SELECT current.week AS "week",
          ...
          FROM gvt_price current,
               gvt_price prev
         WHERE current.week - interval '7 days' = prev.week 
    ),
    -- Calculation of
...
SELECT p.week::date AS "week",
    ...
    SUM(COALESCE(kpis_a.new_tvl, 0) + COALESCE(kpis_a.renewed_tvl) - COALESCE(kpis_a.churned_tvl,0)) OVER (ORDER BY p.week ASC ROWS BETWEEN unbounded preceding AND CURRENT ROW) * gvt_price AS "Total TVL",
